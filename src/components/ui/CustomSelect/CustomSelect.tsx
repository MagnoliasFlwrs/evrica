...
const CustomSelect: React.FC<CustomSelectProps> = ({
    ...
    return (
      <div className={styles.selectedContent}>
        ...
        {values.length === 1 ? (
          ...
          <Flex className={styles.selectedOptions}>
            <span className={styles.selectedCount}>{values.length}</span>
            <button type="button" className={styles.removeButton} onClick={clearAll}>
              <CloseIcon />
            </button>
          </Flex>
        )}
  ...
  return (
    <div className={styles.customSelect} ref={selectRef}>
      <div
        className={`${styles.selectHeader} ${isOpen ? styles.open : ''} ${isEmbedded ? styles.embedded : ''}`}
        onClick={() => setIsOpen(!isOpen)}
        style={{ minWidth: width }}
      >
        <div className={styles.selectedValueContainer}>{getDisplayText()}</div>
      ...
      {isOpen && (
        <div className={styles.selectOptions}>
          ...
          {multiple &&
            searchable && ( // Показываем поиск только если multiple И searchable=true
          ...
          <div className={styles.optionsScroll}>
            {filteredOptions.map((option) => (
              ...
              >
                {tag ? (
                  <Tag className={`${styles.tag} ${styles[option.value]}`}>{option.label}</Tag>
                ) : (
