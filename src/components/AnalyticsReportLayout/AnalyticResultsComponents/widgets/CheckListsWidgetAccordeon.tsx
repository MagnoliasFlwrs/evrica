...
import CheckListsWidgetAccordeonSlide from "./CheckListsWidgetAccordeonSlide";
import {useSwiperManager} from "../../hooks/SwiperManagerContext";
import {swiperData} from "./mockData";
import {measureElementHeight} from "../helpers";

const CheckListsWidgetAccordeon = () => {
    ...
    useEffect(() => {
        slideContentRefs.current = slideContentRefs.current.slice(0, swiperData.length);
    }, [swiperData.length]);
                ...
                <Flex className={styles.CheckListsWidgetAccordeonHeadRowSwiper}>
                    <Swiper
                        ...
                        onSwiper={(swiper: SwiperType) => {
                            registerSwiper('checklists', swiper);

                            if (swiper.activeIndex !== activeIndex) {
                                swiper.slideTo(activeIndex);
                            }
                        }}
                        onDestroy={() => unregisterSwiper('checklists')}
                    >
                        {swiperData.map((item, index) => (
                            <SwiperSlide key={index}>
                                <CheckListsWidgetAccordeonSlide
                                    item={item}
...
export default CheckListsWidgetAccordeon;