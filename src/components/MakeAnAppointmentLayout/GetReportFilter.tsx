...
const GetReportFilter = () => {
  ...
  const setGeneralManagerReportsObj = useAnalyticsStore2(
    (state) => state.setGeneralManagerReportsObj,
  );
  ...
  const isFiltersReady = useMemo(() => {
    return !dateStart || !dateEnd || !selectedCategoryId || !user?.organization_id;
  }, [dateEnd, dateStart, selectedCategoryId, user]);

  const foundCount = useMemo(() => {
    if (
      !managersReportData ||
      Array.isArray(managersReportData) ||
      typeof managersReportData !== 'object'
    ) {
      return 0;
    }
