  ...
  persist<CallsState>(
    (set, get) => ({
      ...
      getPendingCalls: async () => {
        ...
        try {
          const res = await axiosInstanceAll.get(`${baseAuthUrl}/organization/get-pending-calls`, {
            headers: {
      ...
      getCallsCategories: async () => {
        set({ loading: true, error: false });
        try {
          const res = await axiosInstanceAll.get(`${baseAuthUrl}/location`, {
            headers: {
              'Content-Type': 'application/json',
              accept: '*/*',
              authorization: `Bearer ${localStorage.getItem('accessToken')}`,
      ...
      getCategoryDictionaryFullData: async () => {
        ...
        try {
          ...
          const payload = await get().fetchCategoryDictionaryFullData(
            categoryDictionaryFullDataObj,
          );
          if (payload != null) {
            set({
