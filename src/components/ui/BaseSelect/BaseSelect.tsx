...
const BaseSelect: React.FC<BaseSelectProps> = ({
  ...
  return (
    <div className={styles.baseSelect} ref={baseSelectRef} style={{ width }}>
      <div
        className={`${styles.selectHeader} ${variant === 'light' ? styles.light : ''} ${isOpen ? styles.open : ''} ${isOutlined ? styles.outlined : ''} ${isEmbedded ? styles.embedded : ''}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <div
          className={`${styles.selectedValueContainer} ${variant === 'light' ? styles.light : ''}`}
        >
          {getDisplayText()}
        </div>
